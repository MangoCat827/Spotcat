package com.mangocat.spotcat.statusbar
...
import com.mangocat.spotcat.setting.SpotcatSettingsBus
import com.mangocat.spotcat.setting.SpotcatSettingsListener
import com.mangocat.spotcat.setting.SpotcatSettingsState
import com.mangocat.spotcat.spotify.SpotifyState
import java.awt.*
...
class MyStatusBarWidget : CustomStatusBarWidget {
    ...
    // Settings listener to update UI when setting changes
    private val settingsListener = object : SpotcatSettingsListener {
        override fun onSettingsChanged() {
            shouldShowTitle = settings.showTitle
            iconGap = settings.iconGap
            customLabel = settings.customLabel
//            println("update ui")
//            println("iconGap: $iconGap")
            // update UI
            updateCurrentTrack()
//            horizontalStrut = Box.createHorizontalStrut(iconGap)
            changeButtonUi(spotcatButton, false)
            changeButtonUi(prevButton, false)
            changeButtonUi(playPauseButton, true)
            changeButtonUi(nextButton, false)
            changeButtonUi(volumeButton, false)
        }
    }
    ...
    private val controlsPanel = JPanel(FlowLayout(FlowLayout.LEFT, 0, 0))
    private val spotcatButton = JButton()
    ...
    private val iconSize = 14  // Define standard icon size
        ...
        ApplicationManager.getApplication().invokeLater {
            try {
                ...
                if (newPlayIcon != null && newPauseIcon != null) {
                    ...
                    val playPauseIcon = if (isPlaying) newPauseIcon else newPlayIcon
                    playPauseButton.icon = playPauseIcon
...
//    private var horizontalStrut = Box.createHorizontalStrut(iconGap)
...
//        spotifyIconLabel.preferredSize = Dimension(20, 14)
//        try {
...
//        }
        ...
        configureControlButton(spotcatButton, spotcatIcon, SPOTCAT_TEXT) {
//            println("点击了spotyyButton")
            ApplicationManager.getApplication().invokeLater {
                val settings = SpotcatSettingsState.getInstance()
                if (!settings.showTitle && !temporaryShowTitle) {
                    // 临时显示标题
                    temporaryShowTitle = true
                    updateCurrentTrack()

                    // 使用 javax.swing.Timer，且只执行一次
                    val timer = Timer(3000) {
                        temporaryShowTitle = false
                        updateCurrentTrack()
                    }
                    timer.isRepeats = false  // 只执行一次
                    timer.start()
                }
            }
        }
        ...
        separator.preferredSize = Dimension(1, 14)
        ...
        controlsPanel.add(Box.createHorizontalStrut(4)) // Start padding
//        if (shouldShowTitle) {
        controlsPanel.add(spotcatButton)
//        controlsPanel.add(Box.createHorizontalStrut(4))
        controlsPanel.add(trackInfoLabel)
//        controlsPanel.add(Box.createHorizontalStrut(4)) // Reduced gap between text and previous button
//        }
        controlsPanel.add(prevButton)
//        controlsPanel.add(Box.createHorizontalStrut(iconGap))
        controlsPanel.add(playPauseButton)
//        controlsPanel.add(Box.createHorizontalStrut(iconGap))
        controlsPanel.add(nextButton)
//        controlsPanel.add(Box.createHorizontalStrut(iconGap))
        controlsPanel.add(volumeButton)
        controlsPanel.add(Box.createHorizontalStrut(4)) // End padding

        panel.add(controlsPanel, BorderLayout.CENTER)

        for (component in arrayOf(panel, controlsPanel, trackInfoLabel)) {
        ...
        try {
            ...
            ApplicationManager.getApplication().executeOnPooledThread {
                ...
                ApplicationManager.getApplication().invokeLater {
                    ...
                    volumeDialog?.background = Gray._43
    ...
    private var temporaryShowTitle = false
    ...
    private fun updateUIWithState(state: SpotifyState) {
        // Update track info
        val trackInfo = when {
            !state.isRunning -> if (customLabel.isEmpty()) "Spotcat" else customLabel
            state.trackInfo == null || state.trackInfo == "Not playing" -> ""
            else -> if (customLabel.isEmpty())  "${state.trackInfo}" else  "${customLabel} | ${state.trackInfo}"
        }
        trackInfoLabel.text = if (shouldShowTitle || temporaryShowTitle) trackInfo else ""
...
//        }
    ...
    override fun install(statusBar: StatusBar) {
        SpotcatSettingsBus.addListener(settingsListener)
    ...
    override fun dispose() {
        SpotcatSettingsBus.removeListener(settingsListener)
        updateTimer.stop()
        cancelPopupDismissTimer()
        dismissVolumePopup()
    ...
    private class CircularButtonUI(private val alwaysShowBackground: Boolean = false) : BasicButtonUI() {
        ...
        override fun paint(g: Graphics, c: JComponent) {
            ...
            val size = minOf(button.width, button.height) - 6
        ...
        ApplicationManager.getApplication().invokeLater {
            button.apply {
                if (isPrimary) {
                    ...
                    preferredSize = Dimension(20 + iconGap, 28)
                    maximumSize = Dimension(20 + iconGap, 28)
                    minimumSize = Dimension(20 + iconGap, 28)
